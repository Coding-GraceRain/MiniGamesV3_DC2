...
namespace GAME05 {
	...
	private:
		DATA Layer2;
	public:
		LAYER2(class GAME_* game);
		~LAYER2();
		void create();
		void init();
		void update();
		void draw();
		float wx() { return Layer2.wx; }//world position xを返す関数
		float wy() { return Layer2.wy; }//world position yを返す関数
		int chipSize() { return Layer2.chipSize; }//マップチップの一辺のドット数
